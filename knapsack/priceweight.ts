...
class PriceWeight extends k.ProblemSolver {
	public _find(items : k.Item[], maxWeight : number) : k.Knapsack {
		items.sort(function (a : k.Item, b : k.Item) {
			return (a.getPrice() / a.getWeight()) - (b.getPrice() / b.getWeight());
		});
		var combinations = PriceWeight._countCombinations(items.length);
		...
		for (var i = 0; i < combinations; i++) {
			...
			if (PriceWeight._isBetterSolution(knapsack, solution, maxWeight)) {
